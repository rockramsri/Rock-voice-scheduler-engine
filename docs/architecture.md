...
| `outbox_worker.py` | Separate process. Claims due `outbox` rows, writes the EHR, retries/dead-letters. Eval runners call `drain_outbox_once(agency_id)` instead of this loop. See [emr-architecture](emr-architecture.md). |
| `scoring.py` | Deterministic prospect ranking. Hard filters first: excluded ids (already booked in the window, plus the callout nurse), inactive, license not ok, specialty mismatch. Then a weighted sum — specialty 0.30, area 0.25, availability 0.20, reliability 0.15, cost 0.10 — returning the top 4. Availability compares minutes-since-week-start so a 22:00–06:00 window covers an overnight shift. No LLM cost per callout, and every score is explainable. |
| `ladder.py` | Pure policy, no I/O. Lead time picks the plan: RELAXED at 24h or more (SMS, re-SMS, WhatsApp, then voice), NORMAL in between, URGENT at 5h or less (SMS plus WhatsApp immediately, voice 10 minutes later). The plan is re-picked from the *current* lead time on every rung, so a shift drifting closer automatically escalates. Quiet hours: calls and texts allowed between 06:00 and 22:00 agency-local when `quiet_hours_texts` is true. |
| `rungs.py` | Rung execution. Message rungs bump each offer's rung *before* sending (crash-resume never double-contacts) and honor each nurse's channel preferences. The voice rung calls one prospect at a time, marks stale calls (3 minutes) as `no_answer`, retries a transient `dial_failed` in 30s (two strikes → `no_answer`), and when the current scoreboard is exhausted rescoring pulls the next four eligible nurses (cap 3 rounds) before escalating. Declines, no-answers, and hangups call `wake_shift` so the next prospect is due on the following poll; the 3-minute wait stays only while a call is genuinely live. Urgent shifts inside quiet hours still escalate. Fake 555 numbers are skipped, never dialed. |
...
| `webhook.py` | The inbound message server on `:8787`. Routes: `/textbelt-reply` (HMAC-validated), `/sms` (Twilio-signature-validated), `/health`. Signatures fail closed when `PUBLIC_BASE_URL` is empty unless `WEBHOOK_INSECURE_DEV` is set. TextBelt `textId` (and Twilio MessageSid) are recorded in `webhook_receipts` so at-least-once delivery cannot double-apply. A per-phone cap (20 texts / 10 min) sits in front of the LLM path. A widened accept/decline regex handles pending offers first (`yes I can take it`, `sure`, thumbs-up; mixed `yes but no` falls through); an LLM classifier may propose accept only when that regex also matches. Everything else goes to the work plane's SMS agent (no accept tool) with a 12-second budget and a safe fallback reply. |
